#!/usr/bin/env python3
"""Схемы к запросу во ВНИИПО по абзацу третьему п. 4.4.2 СП 1.13130.2020.
...
class Drawing:
    ...
    def render(self, filename):
        ...
        foot = (
            ...
            f'{esc("Приложение к запросу во ФГБУ ВНИИПО МЧС России о разъяснении абзаца третьего п. 4.4.2 СП 1.13130.2020. Размеры в метрах.")}</text>'
        )
        svg = (
...
def fig1():
    d = Drawing(
        "Рисунок 1. Исходная схема и обозначения размеров",
        "Размеры, которые дверь в максимально открытом положении не должна уменьшать. Обозначения применяются на рисунках 2–6.",
        origin=(300, 262),
    )
    base_stair(d)
    ...
    d.callout(0.60, A_LAND + 0.40, 3)

    d.panel([
        ("head", "Разъясняемое положение", [
            "«Двери, выходящие на лестничную клетку, в максимально открытом положении "
            "не должны уменьшать требуемую ширину лестничных площадок и маршей» "
            "(абзац третий п. 4.4.2 СП 1.13130.2020).",
        ]),
        ("callout", (1, DIM), [
            "**Лестничная площадка,** требуемую ширину которой не должна уменьшать дверь. "
            "В каком направлении измеряется эта ширина — **A** от стены до края марша или "
            "**L** вдоль стены — вопрос 4.",
        ]),
        ("callout", (2, DIM), [
            "**Дверь, выходящая на лестничную клетку.** Норма не определяет ни положение "
            "полотна, принимаемое при проверке, ни точки, между которыми выполняется замер.",
        ]),
        ("callout", (3, DIM), [
            "**Лестничный марш** шириной **b**, требуемую ширину которого дверь также "
            "не должна уменьшать (вопрос 5, рисунок 5).",
        ]),
        ("head", "Исходные данные, принятые на рисунках 2–6", [
            "Ширина марша b = 1,20 м, равная требуемой.",
            "Размер площадки A = 1,40 м, размер L = 2,55 м.",
...
def fig2():
    d = Drawing(
        "Рисунок 2. Что считать «максимально открытым положением»",
        "Один и тот же дверной блок в трёх положениях: остающийся размер площадки различается в 2,6 раза.",
    )
    ...
    d.panel([
        ("head", "Вопрос 1: какое положение полотна принимается за максимально открытое?", [
            "Остающийся размер площадки измерен от кромки полотна до края площадки.",
        ]),
        ("callout", (1, DOOR), [
        ...
        ("head", "Следствие", [
            ...
            "разъясняемого требования, положение «до упора» — о его соблюдении.",
        ]),
    ])
...
def fig3():
    ...
    d.panel([
        ...
        ("head", "Следствие", [
            "При требуемой ширине 1,20 м первый вариант даёт вывод о соблюдении требования, "
            "второй и третий — о его нарушении.",
            "",
            "В письме ФГБУ ВНИИПО МЧС России от 10.08.2018 № 4772-13-4-4 по аналогичной норме "
            "п. 4.4.3 СП 1.13130.2009 указано на необходимость учёта устройств для "
            "самозакрывания и других выступающих частей дверного полотна.",
        ]),
    ])
...
def fig4():
    ...
    d.panel([
        ...
        ("head", "Следствие", [
            "Норма требует, чтобы дверь не уменьшала ширину площадки, но не указывает, какая "
            "из перечисленных конструкций ограничивает эту ширину. При требуемом значении "
            "1,20 м первый вариант даёт вывод о нарушении, остальные — о соблюдении требования.",
        ]),
    ])
...
def fig5():
    ...
    d.panel([
        ("head", "Вопрос 5: как определяется ширина марша, уменьшаемая дверью?", [
        ...
        ("head", "Что требуется разъяснить", [
            "1) Считается ли требование нарушенным, если полотно уменьшает ширину марша "
            "только в одном сечении (размер b′), при том что на остальной длине марша "
            "ширина сохраняется требуемой?",
            "",
            "2) Определяется ли уменьшенная ширина как минимальное расстояние между наиболее "
            "выступающей частью двери и противоположным ограждением по перпендикуляру "
...
def fig6():
    ...
    d.panel([
        ("head", "Вопрос 6: как проверяются несколько дверей?", [
            "На площадку выходят два дверных блока; каждый из них в максимально открытом "
            "положении уменьшает ширину площадки.",
        ]),
